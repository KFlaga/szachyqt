...
void SzachyApp::Run()
{
   ...
   // connect(lobby,SIGNAL(graSieciowa(Opcje*)),this,SLOT(graSieciowa(Opcje*)));
    ...
    connect(lobby,SIGNAL(szukajPrzeciwnika(QString)),lacze,SLOT(wyslijWiadomosc(QString)));
...
void SzachyApp::graSieciowa(Opcje * opts)
{
    ...
    connect((QObject*)silnik,SIGNAL(wyslijRuch(QString)), lacze, SLOT(wyslijWiadomosc(QString)));
    connect((QObject*)silnik,SIGNAL(wyslijWynik(QString)),lacze,SLOT(wyslijWiadomosc(QString)));
    connect(lacze,SIGNAL(wygranaPrzezWalkower()),(QObject*)silnik,SLOT(zakonczPrzezWalkower()));
    connect((QObject*)silnik,SIGNAL(przegrajPrzezWalkower(QString)),lacze,SLOT(wyslijWiadomosc(QString)));
    connect((QObject*)silnik,SIGNAL(zmienStatusNaWolny(QString)),lacze,SLOT(wyslijWiadomosc(QString)));
